...
import newYearsImg from "../../../assets/easter-eggs/new-years.png";
...
const LIGHT_COLORS = ["#ffd700", "#e5e7eb", "#60a5fa", "#a78bfa"];
...
// Gentle continuous fall of gold/silver sparks, staggered across the whole
// hold window so new pieces keep drifting in while earlier ones are still
// falling — varying size/opacity/speed reads as sparks at different distances.
...
// Small twinkling firework lights ringing the screen edge.
...
export default function NewYearsSpark({ reducedMotion, onDone }) {
  const sparks = useMemo(() => buildSparks(reducedMotion ? 0 : 40), [reducedMotion]);
  ...
  const staticSparks = useMemo(() => buildSparks(reducedMotion ? 16 : 0), [reducedMotion]);
  ...
  if (reducedMotion) {
    return (
      <div className="egg-fx nye-fx" aria-hidden="true">
        <motion.div
          className="nye-fx-dim"
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0.9, 0.9, 0] }}
          transition={{ duration: REDUCED_TOTAL, times: [0, 0.2, 0.75, 1] }}
        />
        <motion.div
          className="nye-fx-particle-layer"
          initial={{ opacity: 0 }}
  ...
  return (
    <div className="egg-fx nye-fx" aria-hidden="true">
      ...
      {/* ── Sparks + lights + stars — sit behind the image, never obscure it ── */}
      <motion.div
        className="nye-fx-particle-layer"
        initial={{ opacity: 1 }}
      ...
      {/* ── new-years.png — always on top of the particle layer ── */}
      <div className="nye-fx-image-wrap">
        <motion.img
          src={newYearsImg}
          alt="New Years Eve"

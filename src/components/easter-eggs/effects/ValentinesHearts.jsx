...
import valentinesImg from "../../../assets/easter-eggs/valentines.png";
...
const LIGHT_COLORS = ["#ff6b9d", "#ff4d6d", "#ffb3c6", "#ffd166"];
...
// Gentle continuous fall of hearts, staggered across the whole hold window
// so new ones keep drifting in while earlier ones are still falling —
// varying size/opacity/speed reads as hearts at different distances.
...
// Small twinkling lights ringing the screen edge.
...
export default function ValentinesHearts({ reducedMotion, onDone }) {
  const hearts = useMemo(() => buildHearts(reducedMotion ? 0 : 34), [reducedMotion]);
  ...
  const staticHearts = useMemo(() => buildHearts(reducedMotion ? 14 : 0), [reducedMotion]);
  ...
  if (reducedMotion) {
    return (
      <div className="egg-fx val-fx" aria-hidden="true">
        <motion.div
          className="val-fx-dim"
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0.9, 0.9, 0] }}
          transition={{ duration: REDUCED_TOTAL, times: [0, 0.2, 0.75, 1] }}
        />
        <motion.div
          className="val-fx-particle-layer"
          initial={{ opacity: 0 }}
        ...
        >
          {staticHearts.map(h => (
            <span
              key={`sh-${h.id}`}
              className="val-fx-heart"
              style={{ left: `${h.left}%`, top: `${10 + (h.id % 6) * 13}%`, fontSize: h.size, color: h.color, opacity: h.opacity }}
            >
              ♥
            </span>
          ))}
  ...
  return (
    <div className="egg-fx val-fx" aria-hidden="true">
      ...
      {/* ── Hearts + lights + stars — sit behind the image, never obscure it ── */}
      <motion.div
        className="val-fx-particle-layer"
        initial={{ opacity: 1 }}
      ...
      >
        {hearts.map(h => (
          <motion.span
            key={`heart-${h.id}`}
            className="val-fx-heart"
            style={{ left: `${h.left}%`, fontSize: h.size, color: h.color, opacity: h.opacity }}
            initial={{ y: "-8vh", x: 0, rotate: 0 }}
            animate={{ y: "108vh", x: [0, h.sway, 0, -h.sway, 0], rotate: [0, 12, -12, 0] }}
            transition={{
              duration: h.duration,
              delay: PARTICLES_START + h.delay,
              ease: "linear",
              x: { duration: h.duration, delay: PARTICLES_START + h.delay, ease: "easeInOut" },
              rotate: { duration: h.duration, delay: PARTICLES_START + h.delay, ease: "easeInOut" },
            }}
          >
            ♥
          </motion.span>
      ...
      {/* ── valentines.png — always on top of the particle layer ── */}
      <div className="val-fx-image-wrap">
        <motion.img
          src={valentinesImg}
          alt="Valentines"

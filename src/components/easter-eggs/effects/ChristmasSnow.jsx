...
import christmasImg from "../../../assets/easter-eggs/christmas.png";
...
const LIGHT_COLORS = ["#ff6b6b", "#ffd166", "#4ade80", "#60a5fa", "#f472b6"];
...
// Gentle continuous snowfall, staggered across the whole hold window so new
// flakes keep drifting in while earlier ones are still falling — varying
// size/opacity/speed reads as flakes at different distances.
...
// Small twinkling string-lights ringing the screen edge.
...
export default function ChristmasSnow({ reducedMotion, onDone }) {
  const snow = useMemo(() => buildSnow(reducedMotion ? 0 : 40), [reducedMotion]);
  ...
  const staticSnow = useMemo(() => buildSnow(reducedMotion ? 16 : 0), [reducedMotion]);
  ...
  if (reducedMotion) {
    return (
      <div className="egg-fx xmas-fx" aria-hidden="true">
        <motion.div
          className="xmas-fx-dim"
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0.9, 0.9, 0] }}
          transition={{ duration: REDUCED_TOTAL, times: [0, 0.2, 0.75, 1] }}
        />
        <motion.div
          className="xmas-fx-particle-layer"
          initial={{ opacity: 0 }}
        ...
        >
          ...
          {staticLights.map(l => (
            <span
              key={`sl-${l.id}`}
              className="xmas-fx-light"
              style={{ top: `${l.top}%`, left: `${l.left}%`, background: l.color, boxShadow: `0 0 6px 2px ${l.color}` }}
            />
          ))}
        </motion.div>
        <div className="xmas-fx-image-wrap">
          <motion.img
            src={christmasImg}
            alt="Christmas"
            className="xmas-fx-image"
  ...
  return (
    <div className="egg-fx xmas-fx" aria-hidden="true">
      ...
      {/* ── Snow + lights + stars — sit behind the image, never obscure it ── */}
      <motion.div
        className="xmas-fx-particle-layer"
        initial={{ opacity: 1 }}
      ...
      >
        {snow.map(s => (
          <motion.span
            ...
            initial={{ y: "-8vh", x: 0 }}
            animate={{ y: "108vh", x: [0, s.sway, 0, -s.sway, 0] }}
            transition={{
              duration: s.duration,
              delay: PARTICLES_START + s.delay,
              ease: "linear",
              x: { duration: s.duration, delay: PARTICLES_START + s.delay, ease: "easeInOut" },
            }}
          />
        ))}
      ...
      {/* ── christmas.png — always on top of the particle layer ── */}
      <div className="xmas-fx-image-wrap">
        <motion.img
          src={christmasImg}
          alt="Christmas"
